...
/*
 ...
 input_file            : /home/cstevens1/workspace/emu/device_output/out_codegeneration_merged.swagger.json
...
// global variables for path: /dimming
static char g_dimming_RESOURCE_PROPERTY_NAME_dimmingSetting[] = "dimmingSetting"; // the name for the attribute
int g_dimming_dimmingSetting = 30; // current value of property "dimmingSetting" Current dimming value// registration data variables for the resources
...
post_dimming(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  ...
  oc_rep_t *rep = request->request_payload;
  while (rep != NULL) {
    PRINT("key: (check) %s ", oc_string(rep->name));if (strcmp ( oc_string(rep->name), g_dimming_RESOURCE_PROPERTY_NAME_dimmingSetting) == 0)
    {
...
int init;
  ...
  PRINT("file : /home/cstevens1/workspace/emu/device_output/out_codegeneration_merged.swagger.json\n");
